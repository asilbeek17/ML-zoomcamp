...
pop_makes = list(df.make.value_counts().head().index)
...
pop_models = list(df.model.value_counts().head().index)
...
def prepare_X(df):
    ...
    for r in pop_models:
        df['model_%s' % r] = (df.model == r).astype('int')
        features.append('model_%s' % r)
...
def train_linear_regression_reg(xf, y, r = 0.001):
    ones = np.ones(xf.shape[0])
    xf = np.column_stack([ones, xf])
...
categorical_variables = [
    'make', 'model', 'engine_fuel_type', 'transmission_type', 'driven_wheels', 'market_category',
